...
一个基于 Raft 共识算法实现的强一致分布式 KV 存储系统。项目支持 `Put`、`Get`、`Append`、`Delete` 等基础 KV 操作，通过 Raft 日志复制和多数派提交保证多副本数据一致性，并补充了 Leader 重定向、客户端请求去重、WAL 持久化、节点重启恢复、基础 Snapshot 日志压缩和多节点故障测试能力。
