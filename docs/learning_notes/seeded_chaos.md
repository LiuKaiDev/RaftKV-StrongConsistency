...
- Random `get` and `delete` operations may legitimately return `KEY_NOT_FOUND`; those application results are recorded in history and do not fail the chaos run. Unknown application errors, client argument errors, and exhausted transport retries still fail the run.
